CLASS zcl_data_gen_c145lg DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_data_gen_c145lg IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.

    out->write( 'Deleting existing data...' ).

    DELETE FROM ztravel_145lg.

    INSERT ztravel_145lg FROM (
    ...
    DELETE FROM zbooking_145lg.

    INSERT zbooking_145lg FROM (

      SELECT
        FROM /dmo/booking
        JOIN ztravel_145lg ON /dmo/booking~travel_id = ztravel_145lg~travel_id
        JOIN /dmo/travel ON /dmo/travel~travel_id = /dmo/booking~travel_id
        FIELDS
          "client,
          uuid( ) AS booking_uuid,
          ztravel_145lg~travel_uuid AS parent_uuid,
          ...
          ztravel_145lg~last_changed_at AS local_last_changed_at ).

    out->write( 'Adding Booking Supplements data' ).

    DELETE FROM zbsuppl_145lg.

    INSERT zbsuppl_145lg FROM (
      SELECT FROM /dmo/book_suppl AS supp
        JOIN ztravel_145lg  AS trvl ON trvl~travel_id = supp~travel_id
        JOIN zbooking_145lg AS book ON book~parent_uuid = trvl~travel_uuid
